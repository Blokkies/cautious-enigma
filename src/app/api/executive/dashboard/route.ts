...
export async function GET(request: NextRequest) {
  ...
  const events = await Promise.all(eventList.map(async (event) => {
    const [
      ...
      db.select({ count: sql<number>`count(*)` }).from(counts).where(eq(counts.eventId, event.id)),
      db.select({ count: sql<number>`count(*)` }).from(counts).where(and(eq(counts.eventId, event.id), eq(counts.isMatch, true))),
      db.select({ count: sql<number>`count(*)` }).from(counts).where(and(eq(counts.eventId, event.id), eq(counts.isMatch, false))),
      db.select({ total: sql<number>`COALESCE(sum(abs(variance_value)), 0)` }).from(counts).where(and(eq(counts.eventId, event.id), eq(counts.isMatch, false))),
      db.select({ count: sql<number>`count(*)`, total: sql<number>`COALESCE(sum(abs(variance_value)), 0)` }).from(counts).where(and(eq(counts.eventId, event.id), eq(counts.isMatch, false), sql`variance > 0`)),
      db.select({ count: sql<number>`count(*)`, total: sql<number>`COALESCE(sum(abs(variance_value)), 0)` }).from(counts).where(and(eq(counts.eventId, event.id), eq(counts.isMatch, false), sql`variance < 0`)),
